...
export class RefWrapper {
  ...
  constructor(identifiers: string[], private descriptor: ReactThreeRendererDescriptor) {
    ...
    this.descriptor.createInstance = (props: any, rootContainerInstance: any): any => {
      const refWrapperBase = new RefWrapperBase(identifiers);
...
Identifiers: [${Object.keys(refWrapperBase.wrappedRefs).join(", ")}]`);
        ...
        newProps[wrapper.propertyName] = refWrapperBase.getInstance("geometry");
      });

      const instance: any = originalCreateInstance.call(this.descriptor, newProps, rootContainerInstance);
      instance[this.refWrapperSymbol] = refWrapperBase;
      return instance;
    };
  }
